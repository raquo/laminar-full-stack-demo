...
object CodeSnippetsView {
  ...
  def apply(): HtmlElement = {
    div(
      ...
      p("They show the actual pieces of the relevant code, extracted from the codebase at compile time. So, we're getting compile-time data from sbt all the way into Scala.js. For simple data structures, this can be done with the ", a(href("https://github.com/sbt/sbt-buildinfo"), "sbt-buildinfo"), "sbt plugin, and we do this to display the Laminar version number on the home page, but the way we process code snippets gets us static type safety which is not possible with sbt-buildinfo. So, here is how we do this."),
